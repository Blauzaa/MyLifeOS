...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      {/* --- MOBILE BOTTOM NAV (Fixed di Bawah) --- */}
      <nav className="md:hidden fixed bottom-0 left-0 w-full bg-slate-950/90 backdrop-blur-xl border-t border-white/10 z-50 px-2 py-3 flex justify-around items-center">
        {menus.map((menu) => (
          <Link 
            key={menu.href} 
            href={menu.href}
          ...
          >
            ...
            <span className="text-[9px] font-semibold tracking-tight">
                {menu.name.split(' ')[0]} {/* Ambil kata pertama saja agar tidak sempit */}
            </span>
          </Link>
        ))}
        <button 
