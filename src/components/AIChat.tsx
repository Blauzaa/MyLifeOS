...
export default function AIChat() {
  ...
  const { messages, input, handleInputChange, handleSubmit, isLoading }: any = useChat({
    api: '/api/chat',
    maxSteps: 5,
    // Tambahkan log kalau ada error dari server
    onError: (err: any) => {
      console.error("AI CHAT ERROR:", err)
      alert("AI gagal merespon, cek console!")
    }
  } as any)

  // Fungsi bungkus untuk memastikan submit berjalan
  const onSubmitForm = (e: React.FormEvent<HTMLFormElement>) => {
    console.log("Submit ditekan, input:", input);
    e.preventDefault();
    if (!input) return;
    handleSubmit(e);
  };
    ...
    <div className="fixed bottom-20 md:bottom-6 right-6 z-[100]">
      {isOpen ? (
        <div className="bg-slate-900 border border-white/10 w-80 md:w-96 h-[500px] rounded-3xl shadow-2xl flex flex-col overflow-hidden animate-in slide-in-from-bottom-5 text-white">
          ...
          <div className="p-4 bg-blue-600 flex justify-between items-center">
            <div className="flex items-center gap-2">
              <Sparkles size={18} />
              <span className="font-bold text-sm">LifeOS AI Assistant</span>
            </div>
          ...
          {/* Form */}
          <form onSubmit={onSubmitForm} className="p-4 border-t border-white/5 bg-slate-900">
        ...
        <button 
          ...
          className="bg-blue-600 hover:bg-blue-500 text-white p-4 rounded-full shadow-lg shadow-blue-900/20 transition-transform hover:scale-110 active:scale-95"
        >
          <Bot size={28} />
        </button>
      )}
